...
/**
 * Sensor smoothing and map-motion core for map3.
 *
 * Pipeline summary:
 * 1) handleOnSensor receives BLE packets.
 * 2) The hook calibrates a baseline (quaternion preferred, Euler fallback).
 * 3) Relative motion is denoised using median + EMA smoothing.
 * 4) A RAF loop applies map movement based on the selected mapping method.
 *
 * Mapping methods:
 * - euler: legacy yaw/pitch/roll projection.
 * - quaternion: derives projection from relative quaternion.
 * - basic: configurable routing of smoothed yaw/pitch/roll channels.
 * - pd: consumes coordinates generated by the Pure Data patch.
 */
...
/**
 * Runtime tuning exposed in the motion panel.
 * Keep defaults conservative to avoid jumpy camera movement.
 */
...
// PD coordinates older than this threshold are ignored.
...
/**
 * Main hook used by map3 to convert sensor streams into smooth map movement.
 *
 * Inputs:
 * - mapRef: map instance used for reading center and applying easeTo.
 * - onMotionStop: callback fired after stable low-motion stop detection.
 * - tuning: thresholds/routing/smoothing parameters.
 * - pdMapTargetRef: external PD lat/lng target, used only in mappingMethod=pd.
 */
export function useSensorSmoothing(
  ...
  const handleOnSensor = useCallback(
    (data: any) => {
      ...
      // Use the largest axis delta as a single motion intensity metric.
    ...
    function step() {
      ...
      if (
        ...
        if (now - lastMapUpdate >= mapUpdateMs) {
          ...
          if (isPdProjection) {
            // In PD mode, latitude/longitude come from pdMapTargetRef.
