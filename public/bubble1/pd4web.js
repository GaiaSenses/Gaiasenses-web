async function Pd4WebModule(moduleArg = {}) {
  var moduleRtn;
  ...
  if (ENVIRONMENT_IS_NODE) {
    ...
    ENVIRONMENT_IS_WASM_WORKER =
      ENVIRONMENT_IS_WORKER && worker_threads.workerData == "em-ww";
  }
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME =
        "/Users/fmammoli/Developer/bubble/WebPatch/pd4web.data";
      ...
      async function runWithFS(Module) {
        ...
        async function processPackageData(arrayBuffer) {
          ...
          Module["removeRunDependency"](
            "datafile_/Users/fmammoli/Developer/bubble/WebPatch/pd4web.data",
          );
        }
    ...
    loadPackage({
      files: [
        { filename: "/InterRegular.ttf", start: 0, end: 407056 },
        { filename: "/Libs/adenv~.pd", start: 407056, end: 407463 },
        { filename: "/Libs/bubblepattern.pd", start: 407463, end: 407974 },
        { filename: "/Libs/bubblesound.pd", start: 407974, end: 408533 },
        { filename: "/Libs/cycleround.pd", start: 408533, end: 408850 },
        { filename: "/Libs/expcurve~.pd", start: 408850, end: 409275 },
        { filename: "/index.pd", start: 409275, end: 409895 },
        { filename: "/pd.lua", start: 409895, end: 427582 },
        { filename: "/pdx.lua", start: 427582, end: 436397 },
      ],
      remote_package_size: 436397,
    });
  })();
  var arguments_ = [];
    ...
    if (ENVIRONMENT_IS_NODE) {
      ...
      Object.assign(globalThis, {
        addEventListener: (name, handler) =>
          parentPort["on"](name, wrapMsgHandler(handler)),
        removeEventListener: (name, handler) =>
          parentPort["off"](name, wrapMsgHandler(handler)),
      });
    }
    onmessage = (d) => {
      onmessage = null;
      ...
      class WasmAudioWorkletProcessor extends AudioWorkletProcessor {
        ...
        createOutputViews() {
          ...
          for (var n = this.outputViews.length - 1; n >= 0; n--) {
            this.outputViews[n] = HEAPF32.subarray(
              viewDataIdx,
  ...
  var PThread = {
    ...
    allocateUnusedWorker() {
      ...
      if (Module["mainScriptUrlOrBlob"]) {
        ...
        worker = new Worker(new URL("pd4web.js", import.meta.url), {
          type: "module",
          workerData: "em-pthread",
          trackUnmanagedFds: false,
          name: "em-pthread",
  ...
  var ___pthread_create_js = (pthread_ptr, attr, startRoutine, arg) => {
    ...
    transferredCanvasNames = transferredCanvasNames
      ? transferredCanvasNames.split(",")
      : [];
    ...
    for (var name of transferredCanvasNames) {
      ...
      try {
        if (name == "#canvas") {
          if (!Module["canvas"]) {
            err(
              `pthread_create: could not find canvas with ID "${name}" to transfer to thread!`,
            );
            error = 28;
            break;
          }
        ...
        err(
          `pthread_create: failed to transfer control of canvas "${name}" to OffscreenCanvas! Error: ${e}`,
        );
        return 28;
      }
    }
  ...
  var __emscripten_create_audio_worklet = (
    wwID,
    contextHandle,
    stackLowestAddress,
    stackSize,
    ...
    audioWorklet
      .addModule(locateFile("pd4web.js"))
      .then(() => {
    ...
    if (mode == 0) {
      MainLoop.scheduler = function MainLoop_scheduler_setTimeout() {
        var timeUntilNextTick =
          Math.max(0, MainLoop.tickStartTime + value - _emscripten_get_now()) |
          0;
  ...
  var _emscripten_create_wasm_audio_worklet_node = (
    ...
    var opts = options
      ? {
          ...
          channelInterpretation: [, "discrete"][HEAP32[(options + 20) >> 2]],
          processorOptions: {
            callback,
            userData,
  ...
  var _emscripten_create_wasm_audio_worklet_processor_async = (
    ...
    while (numAudioParams--) {
      audioParams.push({
        ...
        automationRate:
          (HEAP32[(audioParamDescriptors + 12) >> 2] ? "k" : "a") + "-rate",
      });
  ...
  function _emscripten_get_device_pixel_ratio() {
    if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 0, 1);
  ...
  var setOffscreenCanvasSizeOnTargetThread = (
    targetThread,
    targetCanvas,
    width,
    height,
  ...
  var findCanvasEventTarget = (target) => {
    ...
    return (
      GL.offscreenCanvases[target.slice(1)] ||
  ...
  function _emscripten_set_orientationchange_callback_on_thread(
    userData,
    useCapture,
    callbackfunc,
    targetThread,
  ...
  function _emscripten_set_window_title(title) {
    if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 0, 1, title);
  ...
  function _emscripten_webgl_create_context_proxied(target, attributes) {
    if (ENVIRONMENT_IS_PTHREAD)
      return proxyToMainThread(17, 0, 1, target, attributes);
  ...
  function _getaddrinfo(node, service, hint, out) {
    if (ENVIRONMENT_IS_PTHREAD)
      return proxyToMainThread(20, 0, 1, node, service, hint, out);
    var addr = 0;
    var port = 0;
    var flags = 0;
    var family = 0;
  ...
  var FS = {
    ...
    readFile(path, opts = {}) {
      ...
      withStackSave(() => {
        ...
        FS.handleError(
          -__wasmfs_read_file(stringToUTF8OnStack(path), bufPtr, sizePtr),
        );
    ...
    msync: (stream, bufferPtr, offset, length, mmapFlags) =>
      FS.handleError(__wasmfs_msync(bufferPtr, length, mmapFlags)),
    ...
    unmount: (mountpoint) =>
      FS.handleError(
        withStackSave(() => _wasmfs_unmount(stringToUTF8OnStack(mountpoint))),
      ),
  ...
  var ASM_CONSTS = {
    ...
    368284: () => window.innerWidth,
    368314: () => window.innerHeight,
    368345: ($0, $1, $2) => {
  ...
  function JS_CreateNumberInput(canvasId) {
    ...
    el.addEventListener("input", () => {
      ...
      el.value = "";
      dispatchFakeKey(digit, "Numpad" + digit, digit.charCodeAt(0));
    });
    el.addEventListener("blur", (e) => {
      dispatchFakeKey("Enter", "Enter", 13);
    });
  }
  function JS_CreateTextInput(canvasId) {
  ...
  function JS_GetMicAccess(audioContext, audioWorkletNode, nInCh) {
  ...
  function JS_SuspendAudioWorklet(audioContext) {
    Pd4WebAudioContext = emscriptenGetAudioObject(audioContext);
  ...
  function assignWasmExports(wasmExports) {
    ...
    _free = wasmExports["uc"];
    _main = Module["_main"] = wasmExports["vc"];
    _pthread_self = wasmExports["wc"];
    ___getTypeName = wasmExports["xc"];
    __embind_initialize_bindings = wasmExports["yc"];
    _ntohs = wasmExports["zc"];
    _htons = wasmExports["Ac"];
    __emscripten_tls_init = wasmExports["Cc"];
    __emscripten_run_callback_on_thread = wasmExports["Dc"];
    __emscripten_set_offscreencanvas_size_on_thread = wasmExports["Ec"];
    __emscripten_thread_init = wasmExports["Fc"];
    ___set_thread_state = wasmExports["Gc"];
    __emscripten_thread_crashed = wasmExports["Hc"];
    _htonl = wasmExports["Ic"];
    __emscripten_run_js_on_main_thread_done = wasmExports["Jc"];
    __emscripten_run_js_on_main_thread = wasmExports["Kc"];
    __emscripten_thread_free_data = wasmExports["Lc"];
    __emscripten_thread_exit = wasmExports["Mc"];
    __emscripten_timeout = wasmExports["Nc"];
    __emscripten_check_mailbox = wasmExports["Oc"];
    ...
    __emscripten_wasm_worker_initialize = wasmExports["Uc"];
    __wasmfs_read_file = wasmExports["Vc"];
    __wasmfs_write_file = wasmExports["Wc"];
    __wasmfs_mkdir = wasmExports["Xc"];
    __wasmfs_rmdir = wasmExports["Yc"];
    __wasmfs_open = wasmExports["Zc"];
    __wasmfs_mknod = wasmExports["_c"];
    __wasmfs_unlink = wasmExports["$c"];
    __wasmfs_chdir = wasmExports["ad"];
    __wasmfs_symlink = wasmExports["bd"];
    __wasmfs_readlink = wasmExports["cd"];
    __wasmfs_write = wasmExports["dd"];
    __wasmfs_pwrite = wasmExports["ed"];
    __wasmfs_chmod = wasmExports["fd"];
    __wasmfs_fchmod = wasmExports["gd"];
    __wasmfs_lchmod = wasmExports["hd"];
    __wasmfs_llseek = wasmExports["id"];
    __wasmfs_rename = wasmExports["jd"];
    __wasmfs_read = wasmExports["kd"];
    __wasmfs_pread = wasmExports["ld"];
    __wasmfs_truncate = wasmExports["md"];
    __wasmfs_ftruncate = wasmExports["nd"];
    __wasmfs_close = wasmExports["od"];
    __wasmfs_mmap = wasmExports["pd"];
    __wasmfs_msync = wasmExports["qd"];
    __wasmfs_munmap = wasmExports["rd"];
    __wasmfs_utime = wasmExports["sd"];
    __wasmfs_stat = wasmExports["td"];
    __wasmfs_lstat = wasmExports["ud"];
    __wasmfs_mount = wasmExports["vd"];
    __wasmfs_identify = wasmExports["wd"];
    __wasmfs_readdir_start = wasmExports["xd"];
    __wasmfs_readdir_get = wasmExports["yd"];
    __wasmfs_readdir_finish = wasmExports["zd"];
    __wasmfs_get_cwd = wasmExports["Ad"];
    _wasmfs_create_jsimpl_backend = wasmExports["Bd"];
    _wasmfs_create_file = wasmExports["Cd"];
    _wasmfs_unmount = wasmExports["Dd"];
  ...
  function assignWasmImports() {
    wasmImports = {
      p: JS_Alert,
      rc: JS_CreateNumberInput,
      qc: JS_CreateTextInput,
      fb: JS_GetMicAccess,
      lb: JS_IsDarkMode,
      Ga: JS_Pd4WebFocusNumberInput,
      Fa: JS_Pd4WebFocusTextInput,
      ua: JS_SuspendAudioWorklet,
      Aa: ___assert_fail,
      $: ___cxa_throw,
      Zb: ___pthread_create_js,
      mc: __abort_js,
      za: __embind_register_bigint,
      ib: __embind_register_bool,
      nb: __embind_register_class,
      mb: __embind_register_class_constructor,
      ...
      ca: __emscripten_create_audio_worklet,
      dc: __emscripten_init_main_thread_js,
      jc: __emscripten_log_formatted,
      Ub: __emscripten_notify_mailbox_postmessage,
      db: __emscripten_proxied_gl_context_activated_from_main_browser_thread,
      Ma: __emscripten_receive_on_main_thread_js,
      Wb: __emscripten_runtime_keepalive_clear,
      Vb: __emscripten_system,
      Ka: __emscripten_thread_cleanup,
      cc: __emscripten_thread_mailbox_await,
      ic: __emscripten_thread_set_strongref,
      Cb: __emscripten_throw_longjmp,
      f: __emval_create_invoker,
      b: __emval_decref,
      D: __emval_get_global,
      _: __emval_get_module_property,
      h: __emval_get_property,
      c: __emval_incref,
      Ea: __emval_instanceof,
      e: __emval_invoke,
      Da: __emval_is_number,
      Ca: __emval_is_string,
      Ba: __emval_new_array,
      g: __emval_new_cstring,
      d: __emval_run_destructors,
      kb: __emval_set_property,
      o: __emval_typeof,
      _b: __gmtime_js,
      $b: __localtime_js,
      ac: __mktime_js,
      Xb: __setitimer_js,
      bc: __tzset_js,
      Pb: __wasmfs_copy_preloaded_file_data,
      Nb: __wasmfs_get_num_preloaded_dirs,
      Ob: __wasmfs_get_num_preloaded_files,
      Lb: __wasmfs_get_preloaded_child_path,
      Jb: __wasmfs_get_preloaded_file_mode,
      Qb: __wasmfs_get_preloaded_file_size,
      Mb: __wasmfs_get_preloaded_parent_path,
      Kb: __wasmfs_get_preloaded_path_name,
      Ib: __wasmfs_jsimpl_alloc_file,
      Ia: __wasmfs_jsimpl_free_file,
      Hb: __wasmfs_jsimpl_get_size,
      Fb: __wasmfs_jsimpl_read,
      Db: __wasmfs_jsimpl_set_size,
      Eb: __wasmfs_jsimpl_write,
      Sb: __wasmfs_stdin_get_char,
      lc: _clock_time_get,
      y: _emscripten_asm_const_int,
      pb: _emscripten_async_call,
      La: _emscripten_check_blocking_allowed,
      aa: _emscripten_create_audio_context,
      cb: _emscripten_create_wasm_audio_worklet_node,
      bb: _emscripten_create_wasm_audio_worklet_processor_async,
      i: _emscripten_date_now,
      C: _emscripten_err,
      hc: _emscripten_exit_with_live_runtime,
      Gb: _emscripten_get_device_pixel_ratio,
      t: _emscripten_get_now,
      va: _emscripten_glActiveTexture,
      Y: _emscripten_glAttachShader,
      X: _emscripten_glBindAttribLocation,
      W: _emscripten_glBindBuffer,
      r: _emscripten_glBindFramebuffer,
      B: _emscripten_glBindRenderbuffer,
      q: _emscripten_glBindTexture,
      ta: _emscripten_glBlendFuncSeparate,
      sa: _emscripten_glBufferData,
      V: _emscripten_glCheckFramebufferStatus,
      G: _emscripten_glClear,
      F: _emscripten_glClearColor,
      A: _emscripten_glColorMask,
      U: _emscripten_glCompileShader,
      ra: _emscripten_glCreateProgram,
      T: _emscripten_glCreateShader,
      qa: _emscripten_glCullFace,
      pa: _emscripten_glDeleteBuffers,
      oa: _emscripten_glDeleteFramebuffers,
      na: _emscripten_glDeleteProgram,
      ma: _emscripten_glDeleteRenderbuffers,
      S: _emscripten_glDeleteShader,
      R: _emscripten_glDeleteTextures,
      u: _emscripten_glDisable,
      Q: _emscripten_glDisableVertexAttribArray,
      n: _emscripten_glDrawArrays,
      w: _emscripten_glEnable,
      P: _emscripten_glEnableVertexAttribArray,
      la: _emscripten_glFinish,
      O: _emscripten_glFramebufferRenderbuffer,
      N: _emscripten_glFramebufferTexture2D,
      $a: _emscripten_glFrontFace,
      _a: _emscripten_glGenBuffers,
      Ya: _emscripten_glGenFramebuffers,
      Xa: _emscripten_glGenRenderbuffers,
      Wa: _emscripten_glGenTextures,
      Za: _emscripten_glGenerateMipmap,
      m: _emscripten_glGetError,
      z: _emscripten_glGetIntegerv,
      Ua: _emscripten_glGetProgramInfoLog,
      Va: _emscripten_glGetProgramiv,
      ja: _emscripten_glGetShaderInfoLog,
      ka: _emscripten_glGetShaderiv,
      M: _emscripten_glGetUniformLocation,
      Ta: _emscripten_glLinkProgram,
      k: _emscripten_glPixelStorei,
      ia: _emscripten_glRenderbufferStorage,
      Sa: _emscripten_glScissor,
      ha: _emscripten_glShaderSource,
      v: _emscripten_glStencilFunc,
      L: _emscripten_glStencilMask,
      x: _emscripten_glStencilOp,
      ga: _emscripten_glStencilOpSeparate,
      Ra: _emscripten_glTexImage2D,
      E: _emscripten_glTexParameteri,
      Qa: _emscripten_glTexSubImage2D,
      Pa: _emscripten_glUniform1i,
      Oa: _emscripten_glUniform2fv,
      Na: _emscripten_glUniform4fv,
      fa: _emscripten_glUseProgram,
      ea: _emscripten_glVertexAttribPointer,
      da: _emscripten_glViewport,
      kc: _emscripten_out,
      Tb: _emscripten_resize_heap,
      ab: _emscripten_resume_audio_context_sync,
      pc: _emscripten_run_script,
      Ha: _emscripten_set_canvas_element_size,
      xb: _emscripten_set_element_css_size,
      K: _emscripten_set_interval,
      ...
      qb: _emscripten_set_orientationchange_callback_on_thread,
      ...
      fc: _emscripten_set_window_title,
      eb: _emscripten_supports_offscreencanvas,
      Yb: _emscripten_unwind_to_js_event_loop,
      ob: _emscripten_webgl_destroy_context,
      xa: _emscripten_webgl_do_create_context,
      wa: _emscripten_webgl_make_context_current_calling_thread,
      ec: _environ_get,
