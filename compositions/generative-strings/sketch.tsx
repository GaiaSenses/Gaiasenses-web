...
export type GenerativeStringsSketchProps = {
  /**
   * `temperature`, e não `temp`, porque é o nome do vocabulário — o mesmo que
   * toda animação usa para este dado. Dentro do sketch a variável continua
   * `temp`; só a fronteira foi padronizada.
   */
  temperature: number;
  play: boolean;
};
...
export default function GenerativeStringsSketch(
  initialProps: GenerativeStringsSketchProps
) {
  ...
  return <P5jsContainer sketch={sketch} temp={Math.abs(temp)}></P5jsContainer>;
}
